...
def thanks(request):
    ...
    # variables = request.GET.get('user provided value for name from the form that was processed through action
    # # to the url that got this function indexed.', 'default value')
    # fname = request.GET.get('firstname','default')
    # lname = request.GET.get('lastname','default')
    # mail = request.GET.get('email','default')
    # country = request.GET.get('country','default')
    # phnum = request.GET.get('phnumber','default')
    # params = [fname,lname,mail,country,phnum]
    return render(request, 'thanks.html')


def welcome(request):
    fullnamee = request.GET.get('fullname', 'default')
    # print(fullnamee)
    try:
...
def analyzed(request):
    user_text = request.GET.get('user_text','default')
    # select box value
    purpose = request.GET.get('purpose','default')
    bold = request.GET.get('bold','default')
